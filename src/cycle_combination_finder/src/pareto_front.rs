...
impl Ord for CCParetoFront {
    fn cmp(&self, other: &Self) -> Ordering {
        self.possible_registers
            .len()
            .cmp(&other.possible_registers.len())
    }
}
...
impl PartialEq for CCParetoFront {
    fn eq(&self, other: &Self) -> bool {
        self.possible_registers.len() == other.possible_registers.len()
    }
}
...
impl CCParetoFront {
    ...
    pub fn push(&mut self, existing: Arc<[u32]>) -> bool {
        for (i, member) in self.possible_registers.iter().enumerate() {
            if dominate(member.iter().copied(), existing.iter().copied()) {
                ...
                if i > 0 {
                    ...
                    unsafe {
                        self.possible_registers.swap_unchecked(i, i - 1);
                    }
                }
                return false;
        ...
        self.possible_registers.push(existing);
        true
    }
    ...
    pub fn merge(&mut self, other_pareto_front: Self) {
        ...
        let mut largest_front = other_pareto_front.possible_registers;
        if largest_front.len() < self.possible_registers.len() {
            std::mem::swap(&mut self.possible_registers, &mut largest_front);
        }
        ...
        std::mem::swap(&mut self.possible_registers, &mut largest_front);
        self.possible_registers.extend(largest_front);
    }
}
