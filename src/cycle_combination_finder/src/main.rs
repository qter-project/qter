...
fn main() {
    ...
    let ccf = CycleCombinationFinder::builder().with_num_cores(NumCores::AllCores);
    if p == "minx3" {
        let minx3 = minxN::MINX3.clone();
        ccf.with_puzzle_def(&minx3)
            .with_register_count(NonZeroU16::new(3).unwrap())
            .with_expected_length_assertion(Some(347))
            .find()
            .unwrap();
    } else if p == "minx4" {
        ...
        ccf.with_puzzle_def(&minx4)
            .with_register_count(NonZeroU16::new(3).unwrap())
            .with_expected_length_assertion(Some(251))
            .find()
            .unwrap();
    } else if p == "cube3" {
        ...
        ccf.with_puzzle_def(&cube3)
            ...
            .with_expected_length_assertion(Some(5))
            .find()
            .unwrap();
    } else {
        println!("Enter minx3 or minx4 or cube3");
