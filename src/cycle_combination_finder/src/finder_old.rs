...
impl<const N: usize> CycleCombinationFinder<N> {
    ...
    fn possible_order_list(
        ...
        while let Some(s) = stack.pop() {
            ...
            let maybe_orbit_def = self.puzzle_def.orbit_defs().iter().find(|&&orbit_def| {
    ...
    fn add_order_to_registers(
        ...
        for combo in &mut *cycle_combos {
            if registers[last_reg as usize].order <= combo.cycles[0].order {
    ...
    pub fn find(
        ...
        let ret = match optimality {
            Optimality::Equivalent => self.find_equivalent(register_count),
