...
impl CycleCombinationFinder {
    pub fn from_ksolve(ksolve: &KSolve) -> Option<Self> {
        ...
        Self::from_orbit_defs(orbit_defs)
    }

    #[must_use]
    pub fn from_orbit_defs(orbit_defs: Vec<OrbitDef>) -> Option<Self> {
        if orbit_defs.is_empty() {
            None
        } else {
            Some(Self { orbit_defs })
        }
    }
            ...
            for i in 0..=max_prime_power.exponent {
                let min_piece_count = if i == 0 {
                    ...
                    if i == 1 {
                        ...
                        // TODO You're dividing the order that you want by the maximum orientation improvement
    ...
    fn possible_order_test(
        ...
        for &orbit_def in &self.orbit_defs {
        ...
        let mut stack: Vec<ComboIteration> = vec![ComboIteration {
            ...
            orbit_sums: vec![0; self.orbit_defs.len()], // pieces used in each orbit
            assignments: vec![vec![vec![]; self.orbit_defs.len()]; registers.len()],
        ...
        while let Some(mut s) = stack.pop() {
            ...
            for (o, &orbit_def) in self.orbit_defs.iter().enumerate() {
    ...
    pub fn find(
        ...
        let ret = match search_strategy {
            Optimality::Equivalent => {
                ...
                let total_pieces = self.orbit_defs.iter().fold(0, |sum, &orbit_def| {
