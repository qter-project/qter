...
//! moves are needed to solve a given position of the Rubik's Cube. The
//! intention is for these tables to be pre-generated before any solving work
//! begins.
//!
//! Each table is generated by executing an iterative deepening DFS (IDDFS)
//! starting from the solved state. For each state, the depth is recorded in a
...
impl<'id, 'a, P: PuzzleState<'id>> OrbitPruningTablesGenerateMeta<'id, 'a, P> {
    ...
    /// # Errors
    ///
    /// Returns an error if the length of `sorted_cycle_structure` does not
    /// match
...
mod tests {
    ...
    //     let cube3_def = PuzzleDef::<Cube3>::new(&puzzle("3x3").ksolve(),
    // guard).unwrap();     let solved = cube3_def.new_solved_state();
    ...
    //             storage_backend:
    // UncompressedStorageBackend::initialize_from_meta(MaxEntries(100)),
    //             orbit_def: cube3_def.sorted_orbit_defs[0],
    //             orbit_identifier: 0,
    //         };

    //     assert_eq!(exact_corners_pruning_table.hash_orbit_state(&solved), 0);
    //     result_1.replace_compose(&solved, &u_move.puzzle_state,
    // &cube3_def.sorted_orbit_defs);     assert_eq!(
    ...
    //         &cube3_def.sorted_orbit_defs,
    //     );
    //     assert_eq!(exact_corners_pruning_table.hash_orbit_state(&result_2),
    // 0);

    //     // shortest 11 cycle alg
    //     result_1 = apply_moves(&cube3_def, &solved, "U R U F L R' U' R' F'
    // D'", 1);

    //     assert_eq!(exact_corners_pruning_table.hash_orbit_state(&result_1),
    // 0);     result_2.replace_compose(
    //         &result_1,
    //         &u_move.puzzle_state,
    //         &cube3_def.sorted_orbit_defs,
    //     );
