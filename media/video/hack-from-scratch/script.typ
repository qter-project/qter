#import "@local/blockst:0.1.0": blockst, scratch, set-blockst
#import scratch.en : *
...
And we'd also like to mention collaborators Daniel and Neel.
...
First, we need some way to represent numbers on a Rubik's cube. To do this, we will suppose that the solved cube represents the number zero. Next, we know that the most basic operation you can do to a Rubik's cube is to make turns, so perhaps we can suppose that doing turns performs addition. For example, lets say that turning the up face represents addition by one. Then this represents one, then two, then three, then four? You can see that it loops back around to zero. It's analagous to how the numbers on a clock loop around after reaching twelve.
...
#blockst[
    #change-variable-by("A", "1")
]

You may notice that the instruction starts with the number zero. This is because all of the instructions get numbers as labels, and we'll explain why later. After the bar character, each of these symbols represent moves has a letter representing which face to turn. If the letter was on its own, you would turn the face clockwise. If the letter has a `2`, you would turn the face twice. If it has a `'`, you would turn it counterclockwise.
...
#blockst[
    #change-variable-by("A", "5")
    #change-variable-by("B", "3")
]
...
```
#blockst[
    #ask-and-wait("Which Fibonacci number to calculate:")
    #change-variable-by("A", custom-input("Answer"))
]
...
```q
0 | solved-goto DFR FR 2
1 | U R U' D2 B
2 | (rest of program)
```
#blockst[
    #if-then(op-not(equals(custom-input("A"), "0")))[
        #change-variable-by("A", 1)
    ]
]
...
```
#blockst[
    #repeat-until(equals(custom-input("A"), "0"))[
        #change-variable-by("A", "-1")
        #change-variable-by("B", "1")
    ]
]

What this does is repeatedly subtract one from register A and add one to register B. This sequence of moves has the same effect as those two addition operations. Once `A` is zero, the `solved-goto` makes us exit the loop, and `B` has been set to `B + A`. This pattern of "do something until a register is zero" is so common that we have our own instruction for it, called `repeat-until`.
...
```
#blockst[
    #say(join("The number is ", custom-input("A")))
    #stop("all")
]
